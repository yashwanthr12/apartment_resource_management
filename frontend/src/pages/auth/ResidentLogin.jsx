...
export default function ResidentLogin() {
  ...
  useEffect(() => {
    document.title = 'Resident Login | ApartEase';
  }, []);
  ...
  const handleSendRecoveryOtp = async (e) => {
    ...
    const { ok, status, data } = await apiPost('/api/auth/send-otp', {
      ...
      purpose: 'forgot-password-resident'
    });
    setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    const { ok, status, data } = await residentLogin(email.trim(), password);
    ...
    if (ok) {
      ...
      login({ ...data.resident, role: 'resident' });
      setTimeout(() => navigate('/resident/dashboard'), 600);
    } else {
      // Special handling for 403 (unverified or deactivated)
      const alertType = status === 403 ? 'warning' : 'danger';
      setAlert({ message: data.error || 'Login failed', type: alertType });
      setLoading(false);
    }
  };
        ...
        <div className="glass-card auth-card">
          ...
          {forgotStep === 0 && !pendingRecovery && (
            <div className="auth-tabs">
              <Link to="/admin/login" className="auth-tab-btn">Admin</Link>
          ...
          {forgotStep === 0 && (
            <>
              ...
              <p className="auth-subtitle">Sign in to view your bills and payments</p>
            </>
          )}
          ...
          {pendingRecovery ? (
            ...
            <form onSubmit={handleSubmit} autoComplete="off">
              <div className="mb-3">
                ...
                <input
                  type="email" className="form-control" id="res-email"
                  placeholder="resident@example.com" required
                  value={email} onChange={(e) => setEmail(e.target.value)}
                />
              </div>
              <div className="mb-4">
                <label className="form-label" htmlFor="res-password">Password</label>
                <div className="password-wrapper">
                  <input
                    type={showPwd ? 'text' : 'password'} className="form-control" id="res-password"
                    placeholder="Enter your password" required style={{ paddingRight: '44px' }}
          ...
          {forgotStep === 0 && (
            <div className="text-center mt-4">
              ...
              {' '}<Link to="/resident/register" style={{ fontSize: '13px', fontWeight: 600 }}>Register here</Link>
            </div>
          )}
        </div>
      </div>
